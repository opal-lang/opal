...
The Agent Skills specification has emerged as a cross-industry standard (adopted by GitHub Copilot, AI agent, Cursor, OpenAI Codex, and others) for extending AI agent capabilities. This creates both opportunity and risk:
...
AI generates:
─────────────────────────────────────────
// deploy.opl - Generated by AI agent
deploy: {
    var VERSION = "1.2.3"
    var NAMESPACE = "production"
...
class OpalIntegration {
  async execute(userIntent: string): Promise<Result> {
    // Generate Opal script
    const opalScript = await this.agent.generate({
      prompt: userIntent,
...
- **External Inspiration:**
  - [AI agent](https://example.com/ai-agent): AI agent with approval workflows
