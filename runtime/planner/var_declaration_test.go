...
// - Planning: Plan stores DisplayID (e.g., "sigil:3J98t56A")
...
// 1. Plan should show: `echo "Hello, sigil:3J98t56A"`
// 2. NOT: `echo "Hello, Aled"`
// 3. Plan output contains `sigil:` placeholder
...
echo "Hello, @var.NAME"`
	...
	// The plan formatter needs to see: echo "Hello, sigil:3J98t56A"
	...
	// ASSERT: Command should contain "sigil:" placeholder
	if !strings.Contains(commandArg, "sigil:") {
		t.Errorf("FAIL: Command should contain DisplayID placeholder 'sigil:', got: %s", commandArg)
	}
...
// - Command strings contain DisplayID: echo "Hello, sigil:3J98t56A"
...
echo "Hello, @var.NAME"`
	...
	if !strings.Contains(formatted, "sigil:") {
		t.Errorf("Formatted plan should contain DisplayID 'sigil:', got:\n%s", formatted)
	}
...
echo "Goodbye, @var.NAME"`
	...
	for i, use := range result.Plan.SecretUses {
		...
		if !strings.Contains(use.DisplayID, "sigil:") {
			t.Errorf("SecretUse[%d].DisplayID should contain 'sigil:', got %q", i, use.DisplayID)
		}
